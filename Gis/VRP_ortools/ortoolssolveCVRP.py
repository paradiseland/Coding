...
def create_data_model(infomation, dist, trunk):
    ...
    data['demands_volume'] = infomation['demand_volume']
    data['vehicle_capacities'] = [trunk[0]]*len(dist)
    data['vehicle_volume_capacities'] = [trunk[1]] * len(dist)
    data['num_vehicles'] = len(dist)
    data['depot'] = 0
    return data
...
def add_capacity_volume_constraints(routing, data, demand_volume_callback):
...
def print_solution(data, manager, routing, assignment, loc, prob):
    ...
    print(Tour)
    plot2(Tour,location=loc, load=Load, volume=Volume, min=total_distance, prob=prob)
...
def main(porb):
    ...
    def demand_volume_callback(from_index):
        """return the demand volume of the node."""
        from_node = manager.IndexToNode(from_index)
    ...
    routing.AddDimensionWithVehicleCapacity(
        ...
        0, 
        data['vehicle_volume_capacities'], 
        True, 
        'Capacity_volume')
    ...
    if assignment:
        print_solution(data, manager, routing, assignment, loc, prob)
