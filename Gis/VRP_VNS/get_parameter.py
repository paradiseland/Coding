...
def get_parameters(file_name):
    ...
    sheet_customers = [sheet_10customers, sheet_30customers, sheet_80customers, sheet_300customers]
    num_of_customer = [10, 30, 80, 300]
    location = []
    demand_weight = []
    demand_volume = []
    for index,sheet in enumerate(sheet_customers):
        location.append([(m, n) for m, n in zip(sheet.col_values(1), sheet.col_values(2))])
    ...
    for i in range(len(info)):
        info[i] = {"num_of_customer":num_of_customer[i],"location":location[i], "demand_weight":demand_weight[i], "demand_volume":demand_volume}
    trunks_type = [(m,n) for m,n in zip(sheet_trunks.col_values(1),sheet_trunks.col_values(2))]
    del trunks_type[0]
