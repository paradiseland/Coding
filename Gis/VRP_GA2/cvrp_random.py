...
def draw_plot(data, generation):
	...
	# X axis labels
	chart.set_axis_labels(Axis.BOTTOM, [str(x) for x in range(1, generation+1)][::14])
	chart.download('plot.png')


def get_route():
	global route
	lst = [i for i in range(num_cities)]
	for i in range( int(ceil(num_cities / float(capacity))) ):
		track_route = []
...
def draw_image(img_file, coords, total_route, start_score, best_score, gen_num):	
	...
	for track_route in total_route:					
		color = COLORS[k]
		k = (k + 1) % len(COLORS)		
		for i in range(len(track_route)):			
...
class Individual:
	...
	def _makechromosome(self):
		...
		lst = [i for i in range(1,num_cities)]
...
def main_run():
	...
	#cm = matrix
	ev = Environment(size=100, maxgenerations=10)
	ev.run()
